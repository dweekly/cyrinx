...
**Relationship to the existing plans.** This is the execution of [EXPERIMENTS.md](../EXPERIMENTS.md) Tier 1 — E1 distance sweep, E2 orientation and surface, E3 ambient noise — whose designs exist and whose result tables are empty. It does not supersede them: the positions below are E1/E2 cells under session labels, they reuse E1's `ds` delay-spread column, and results are written back into those tables rather than into a second ledger. It is also not a second delivery path. Every endpoint capability it needs is an existing numbered item in the [3.0 delivery plan](../CYRINX_3_PLAN.md), whose status ledger records C3-06 through C3-27 as not merged; Stages 5 and 6 below therefore hand measured inputs to those items instead of building a parallel transfer stack in `scratch/`.
...
| 1G | **Waveform-class gate:** strong-tap delay spread against the guard of every expressible profile | Decides whether Stages 2–4 or Stage 8 is next, per position | None; a decision on already recorded data |
...
| 5 | Minimal on-device transfer with selective repair | Translate PHY gains into completed messages | One 15–30 minute session |
| 6 | Conservative automatic profile selection | Let the actual link choose among measured winners | One 20–30 minute mixed-condition session |
...
Stages 3, 4, and 8 can change order when diagnostics identify a specific dominant failure. Bring one second-phone check into the first finalist comparison when a phone is available; Stage 7 expands that check into transfer and handling tests. Stage 7's movement screen moves earlier if ordinary handling breaks the link. Stage 1G is a hard gate rather than advice. For any position whose measured strong-tap spread exceeds every guard the registry can express, Stages 2, 3, 4, and 9 cannot recover it and Stage 8 is that position's next stage; positions that pass the gate continue through the sequence normally. Do not proceed through the sequence mechanically.
...
- decode with the selected C binary and verify bytes at their scheduled positions;
...
- **A conservative profile.** Line for line, the five rows in [the profile registry](../../Sources/CCyrinx/cyrinx_profiles.c) are 16-QAM or 64-QAM; there is no QPSK profile and no rate-1/2 profile in the registry at all, and the longest guard is 768 samples (16 ms at 48 kHz, already 37.5% overhead at NFFT 2048) with the remaining four at 240 and 96 samples. The conservative arm of the Stage 1 map therefore has nothing to run. Authoring it means a new registry row with an explicit classification, which is C3-04 surface and takes the normal review path, not a locally patched constant. Note while authoring it that entry 13's 35.8 ms strong-tap spread would need a guard near 1680 samples at NFFT 2048, and that NFFT 4096 with a 43 ms guard was already measured at zero blocks in that geometry — so "long guard" has a firm ceiling and the gate at Stage 1G exists precisely because widening the guard is not an available answer.
- **A usable delay-spread readout.** [characterize.py](../../scratch/hw20k/characterize.py) already measures delay spread from an exponential sine sweep, but reports the −30 dB point, which NEGATIVE_FINDINGS entry 1 identifies as the wrong basis for a guard decision. Add −10 dB and −20 dB readouts so the number is comparable with entry 13's. This is a separate stimulus per position with its own airtime, not a free by-product of the sync chirp; budget it into the session and treat it as a G2 deliverable that Stage 1 consumes.
...
**Score a completion endpoint from Stage 1 onward, not from Stage 5.** The [review](acoustic-throughput-review-2026-09-04.md) puts completion probability by deadline and session goodput ahead of first-pass block recovery, and Stage 4's own stopping rule concedes that a near-perfect baseline may miss too few blocks to resolve a halving. Both problems have the same cheap fix: the runner already retains ordered per-block results, so selective-repeat ARQ can be replayed over a recorded capture offline. Report "would a 4 KiB message have completed within 15 s over this capture" as a derived endpoint on every attempt from Stage 1. It is a simulation of the protocol, not a measured transfer, and must be labeled as such — but it gives the stages before Stage 5 an endpoint that can move when block recovery cannot.
...
This stage does not author a transfer stack. The capability it describes is [C3-14](../CYRINX_3_PLAN.md) and C3-15 for the PCM adapters, C3-17 for the serialized session engine, C3-19 for manual-profile messages, and C3-22 for selective ARQ, none of which are merged today. Building a second half-duplex transfer with its own sequence numbers and repair logic in `scratch/garage/` would create a competing implementation before the first one exists, and would then have to be reconciled. What this stage owes Phase D is the measured input those items need: the profile set that survived Stages 1–4, the observed repair rates and block-loss distributions per condition, and the retained captures to replay against. What it takes back is the first real completion measurement.
...
- If the receiver remains expensive, fragile, or inefficient, construct one pulse-shaped single-carrier QAM challenger with adaptive DFE or block frequency-domain equalization. Compare using the same chosen modern code, occupied band, declared drive class, and complete overhead accounting. It requires its own digital fixtures and actual transmitted captures.
...
**The first four operator sessions should be enough to choose a direction.** They should be separated by software analysis rather than scheduled back-to-back:
...
| 2b, 20–30 min | Same for C2 and C3, then evaluate the Stage 1G gate | Small baseline map, the waveform-class decision, and three diagnostic conditions |
...
|---|---|---|
| G0 | Conservative registry profile with an explicit classification; −10/−20 dB delay-spread readout in `characterize.py` | Registry serialization and layout tests; sweep readout checked against the entry 13 figures |
| G1 | Capture manifest, runner, immutable attempt ledger, replay command | Existing fixtures plus malformed capture and scheduled-attribution checks |
...
| G7 | Small adaptive profile controller | Deterministic link-state transitions, stale estimates, recovery, later physical checks |
...
**The first implementation task is G0, then G1, then the baseline map.** The order matters because the conservative arm of the baseline map cannot run until a conservative profile exists, and the Stage 1G gate cannot be evaluated until the delay-spread readout reports a strong-tap figure. No exotic DSP should precede the ability to retain a short failed recording, replay it reliably, and explain where information was lost. Once that loop exists, the garage becomes sufficient for making the next several engineering decisions without requiring repeated manual setup or a formal physical lab.
