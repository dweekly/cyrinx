...
vectors; the library's own codec also decodes over the air byte-verified,
though so far only at QPSK rate 1/2 (12.8\,kbps) under a weaker bench coupling
rather than at the 36\,kbps headline, and it ships with an SNR/delay-spread MCS
sounder and a channel-metric-driven user repositioning guide. We frame the result accordingly: this is a well-instrumented
...
on-device decoder; the portable-C library codec (Section~\ref{sec:library}) was
validated digitally against golden vectors but not yet over the air. To close
that gap we exercised the library code path itself end-to-end: a ctypes loader
binds the compiled \texttt{libcyrinxbulk} dylib, the Mac \emph{C-encodes} a frame
and plays it, the Pixel records, and the same C codec \emph{decodes the live
capture}. In this bench session the acoustic coupling was markedly weaker than
the historical palm-rest geometry (received peak $\sim$0.08, EVM $\sim$0.30,
i.e.\ $\sim$10--11\,dB SINR versus the 30\,dB+ of the headline runs), so this is
a code-path validation, not a peak-rate measurement.

At that coupling, \textbf{QPSK rate 1/2 decoded fully byte-verified through the
library on both ends: 25/25 blocks across 3 frames, 12.8\,kbps}. Pushing the
ladder higher at the \emph{same} weak coupling was SNR-limited, not
codec-limited: 16-QAM rate 1/2 recovered 40--49 of 50 blocks ($\sim$20--25\,kbps
with a few CRC failures) and 16-QAM rate 3/4 only 25 of 75. That the C demapper
and Viterbi decoder partially recover live 16-QAM at $\sim$10\,dB SINR is the
point---the ceiling here is the physical link in this particular bench setup, not
the library. This is precisely the regime the adaptive sounder
(Section~\ref{sec:adaptive}) is built to detect and back off from, with ARQ
mopping up the stragglers. Re-running the full palm-rest geometry through the
library to reproduce the 36\,kbps figure natively remains open.
...
binding and decodes over the air through that path
(Sections~\ref{sec:library},~\ref{sec:libota}), but it is \emph{not} yet wired
into the project's streaming transport/MAC API, and the 36\,kbps headline has not
been reproduced natively through the library (the library-native OTA to date is
the weaker-coupling 12.8\,kbps bench result); (ii) is \emph{not}
...
\begin{enumerate}
  ...
  \item A measurement can be made reusable without losing its rigor: the full
        ...
        through the library itself. On top sit two decision surfaces---an
