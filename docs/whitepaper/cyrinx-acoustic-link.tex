...
by the full airtime span including preambles,
...
vectors; the library's own codec decodes over the air byte-verified end to end,
and ships with an SNR/delay-spread MCS sounder and a channel-metric-driven user
repositioning guide. We frame the result accordingly: this is a well-instrumented
...
often want confidentiality and authentication; the predecessor stack carries an
X25519/CTR-HMAC envelope for exactly this. Whether it is worth enabling, however,
depends sharply on the chosen MCS, and the bulk PHY makes that tradeoff explicit
rather than hard-coding it. Computed from the real codec geometry, a per-frame
AEAD tag (28\,B) plus a one-time handshake (80\,B) costs about \textbf{0.15\% of
goodput at 16-QAM r3/4} ($\sim$38\,kbps, large frames) but as much as
\textbf{87.5\% at the multitone-FSK floor} with the tiny frames that regime
implies; the handshake amortizes in $\sim$17\,ms at the top tier versus
$\sim$2.4\,s at the floor. The guidance to a library consumer is therefore
conditional: enable it essentially for free on the fast tiers, and on the floor
either batch larger frames, authenticate without encrypting, or omit it---a
decision the application makes from the same channel quality the sounder already
measures. The envelope is experimental and unaudited; the point here is that its
\emph{cost} is now a documented function of channel quality, not a fixed tax.
...
and Research Challenges.'' CRI-Lab.
