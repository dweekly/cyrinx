...
\textbf{Limitations of the result as it stands.} The measurement is a single
physical geometry (phone on palm rest), one device pair, one ambient
condition, and an open-loop MCS chosen from a same-day channel measurement.
The link is batch (record, then decode), half-duplex, unencrypted, and has no
link layer. The numbers should be read as ``what this hardware pair can do at
contact range,'' not as a general-environment claim.
...
\textbf{The inaudible-ultrasonic variant.} The original goal (and the obvious
product form) is an inaudible link in the 18.5--23.5\,kHz band, and this is
under investigation. The measured channel makes its shape clear in advance:
Mac$\rightarrow$Pixel has 36--40\,dB SNR in that band, so on the order of
15--25\,kbps is plausible in principle; but Pixel$\rightarrow$Mac has only
4.6--10\,dB SNR above 18\,kHz, supporting perhaps 1--3\,kbps. A symmetric-rate
inaudible link is physically unreachable with this phone hardware; the design
must be asymmetric (e.g.\ a fast inaudible downlink with a slow inaudible
return channel, or an audible return burst). These rate figures are estimates
from measured SNR, not yet measured goodput.
