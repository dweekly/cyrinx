...
public final class AcousticCalibration: Sendable {
    ...
    public static func optimizeHardwareVolumes() {
        ...
        do {
            ...
            print("[AcousticCalibration] Warning: Failed to apply native volume overrides: \(error.localizedDescription)")
        }
        #else
        print("[AcousticCalibration] Platform auto-gain calibration is not supported on this platform. Please stage gains manually.")
        #endif
    }
    ...
    public static func calculateTHD(samples: [Float], sampleRate: Double, fundamentalHz: Double) -> Float {
        ...
        guard let fft = try? vDSP.DiscreteFourierTransform(
            ...
            print("[AcousticCalibration] THD failed: Accelerate discrete fourier transform initialization error.")
            return 0.0
        }
        ...
        guard fundamentalBin > 0, fundamentalBin < n / 2 else {
            print("[AcousticCalibration] THD failed: fundamental frequency \(fundamentalHz) Hz lies out of Nyquist bounds.")
            return 0.0
        }
    ...
    public static func calibrateOptimalTxGain(
        ...
        for g in gains {
            if let samples = loopbackSamples[g] {
                ...
                print("[AcousticCalibration] Test Gain: \(g) | Local Loopback THD: \(String(format: "%.2f", thd))%")

                if thd < 5.0 {
                    optimalGain = g
                } else {
                    print("[AcousticCalibration] Transducer amplifier saturation detected at Gain = \(g) (THD >= 5%)")
                    break
                }
            }
        }
    ...
    private static func getDefaultDevice(selector: AudioObjectPropertySelector) throws -> AudioDeviceID {
        ...
        guard status == noErr else {
            throw NSError(
                ...
                code: Int(status),
                userInfo: [NSLocalizedDescriptionKey: "Failed to resolve default device for selector \(selector)"]
            )
        }
        return deviceID
    }
    ...
    private static func setVolumeScalar(deviceID: AudioDeviceID, isInput: Bool, volume: Float) throws {
        ...
        guard status == noErr else {
            throw NSError(
                domain: "Cyrinx.AcousticCalibration",
                code: Int(status),
                userInfo: [NSLocalizedDescriptionKey: "Failed to write hardware volume property for device \(deviceID)"]
            )
        }
    }
    ...
    private static func getVolumeScalar(deviceID: AudioDeviceID, isInput: Bool) throws -> Float {
        ...
        guard status == noErr else {
            throw NSError(
                domain: "Cyrinx.AcousticCalibration",
                code: Int(status),
                userInfo: [NSLocalizedDescriptionKey: "Failed to read hardware volume property for device \(deviceID)"]
            )
        }
        return volume
    }
